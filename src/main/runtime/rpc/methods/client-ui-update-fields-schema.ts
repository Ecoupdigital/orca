...
const UiUpdateFields = z
  .object({
    ...
    // Why: EcoUp per-pane explorer (expanded/width) persists through ui.set;
    // without it the RPC parity guard fails and paired clients reject the whole payload.
    paneExplorerByWorktree: z
      .record(
        z.string(),
        z
          .object({
